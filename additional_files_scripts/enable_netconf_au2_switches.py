...
with open("au2_edge_switches.txt") as f:
    data = f.read()
...
for d in data:
    ...
    ssh.send_config_from_file(config_file = "enable_netconf.txt", strip_prompt=False, strip_command=False, cmd_verify=True, exit_config_mode=False)

    print(f"{datetime.datetime.now()} committing the config on {d}")
    ssh.commit(comment = "Interface-mgmt-In acl addition for netconf", and_quit = True)
